...
class VTuberRenderer:
    ...
    def load_assets(self):
        ...
        self.assets = {
            ...
            # --- LOGIKA ASET MATA BARU ---
            "eyes_left": { 
                "normal": self.load_img("Mata_Kanan_3.png"), # Standby (Lebar)
                "squint": self.load_img("Mata_Kanan_2.png"), # Agak Merem
                "blink":  self.load_img("Mata_Kanan_1.png"), # Merem Total
            },
            "eyes_right": {
                "normal": self.load_img("Mata_Kiri_3.png"),  # Standby (Lebar)
                "squint": self.load_img("Mata_Kiri_2.png"),  # Agak Merem
                "blink":  self.load_img("Mata_Kiri_1.png"),  # Merem Total
            },
            "mouth": {
    ...
    def render(self, state):
        ...
        # Mata Independen
        l_mode = state["eyes_left"]
        l_asset = self.assets["eyes_left"].get(l_mode)
        if l_asset is not None: canvas = self.transform_and_overlay(canvas, l_asset, p["left_eye"])

        r_mode = state["eyes_right"]
        r_asset = self.assets["eyes_right"].get(r_mode)
        if r_asset is not None: canvas = self.transform_and_overlay(canvas, r_asset, p["right_eye"])

        mouth_mode = state["mouth"]
        mouth = self.assets["mouth"].get(mouth_mode)
...
def main():
    ...
    print(">> VTUBER FINAL (SPLIT CONTROLS) <<")
    print(">> 'O' : OPEN Control Panel (Buka Slider)")
    print(">> 'X' : EXIT Control Panel (Tutup Slider)")
    print(">> 'S' : SAVE Config")
    ...
    while True:
        ...
        if show_panel:
            # Pastikan Jendela Ada
            if cv2.getWindowProperty("Control Panel", cv2.WND_PROP_VISIBLE) < 1:
                cv2.namedWindow("Control Panel")
                cv2.resizeWindow("Control Panel", 400, 750)
                # Re-create trackbars if window was closed manually
                for part in ["L Eye", "R Eye", "Mouth"]:
                    cv2.createTrackbar(f"{part} X", "Control Panel", 0, 2000, nothing)
                    cv2.createTrackbar(f"{part} Y", "Control Panel", 0, 2000, nothing)
                    cv2.createTrackbar(f"{part} Rot", "Control Panel", 180, 360, nothing) 
        ...
        if key == ord('q'): break
            ...
            show_panel = True
            last_pose = "force_update"
        elif key == ord('x'): # X for CLOSE/EXIT PANEL
            show_panel = False
